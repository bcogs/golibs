...
func newPair(s string, e error) pair { return pair{s, e} }
...
func TestSingletonMapBasics(t *testing.T) {
	...
	assert.Equal(t, "1", sm.GetOrCreate(1, createlog.createWithKey))
	assert.Equal(t, newPair("3", error(nil)), newPair(sm.GetOrCreateOrFail(3, createlog.createWithKeyAndSucceed)))
	_, err := sm.GetOrCreateOrFail(4, createlog.createWithKeyAndFail)
	assert.Error(t, err)
	assert.Equal(t, newPair("4", error(nil)), newPair(sm.GetOrCreateOrFail(4, createlog.createWithKeyAndSucceed)))
	assert.Equal(t, newPair("3", error(nil)), newPair(sm.GetOrCreateOrFail(3, createlog.createWithKeyAndSucceed)))
	assert.Equal(t, newPair("3", error(nil)), newPair(sm.GetOrCreateOrFail(3, createlog.createWithKeyAndFail)))
...
func TestSingletonMapRaces(t *testing.T) {
	...
	wg.Wait()
	for i := 1; i <= P; i++ {
		assert.Equal(t, newPair(strconv.Itoa(i), error(nil)), newPair(sm.GetOrCreateOrFail(i, createlog.createWithKeyAndFail)))
	}
